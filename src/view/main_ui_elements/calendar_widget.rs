...
impl CalendarWidget {
    ...
    fn create_day_widgets(&mut self, month_data: MonthData) {
        ...
        important_days.iter().for_each(|day| {
            let container = Container::new(DayWidget::new(
                *day,
                chores_data.get(day).cloned().unwrap_or_default(),
            ))
...
impl DayWidget {
    ...
    fn reset(&mut self) {
        ...
        self.column = Flex::column().with_child(Padding::new((0.0, 10.0, 0.0, 0.0), label));
        ...
        self.chores.iter().for_each(|rec| {
            ...
            self.column
                .add_child(Padding::new((5.0, 5.0, 5.0, 0.0), container));
        });
    }
}
...
pub fn build_calendar_widget(
    read_only_database_service: Rc<dyn ReadOnlyDatabaseService>,
) -> impl Widget<DatabaseData> {
