...
const ITEM_LIST_WIDTH: f64 = 500.;
const PERSON_NAME_FIELD_WIDTH: f64 = 100.;
...
pub fn build_list_of_people() -> impl Widget<MainStateData> {
    Flex::column()
        ...
        .with_child(
            Scroll::new(
                List::new(|| {
                    Label::new(|item: &PersonRecordWrapper, _env: &_| item.person_name.clone())
                        .padding((0., 10., 0., 0.))
                })
            ...
            Scroll::new(
                List::new(|| {
                    Label::new(|item: &ChoreTypeRecordWrapper, _env: &_| {
                        format!(
                            "{}\n{}",
                            item.chore_name.clone(),
                            item.chore_description.clone()
                        )
                    })
                    .padding((0., 10., 0., 0.))
                })
